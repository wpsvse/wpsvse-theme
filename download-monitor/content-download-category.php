...
<div class="file-info">
	...
	<?php echo $dlm_download->get_the_image( 'thumbnail' ); ?><?php $dlm_download->the_title(); ?> <?php echo $dlm_download->get_the_version_number();?><?php if ( get_post_meta($dlm_download->id, '_dlm_meta_extra', true) !== '') { echo ' <p>' . get_post_meta($dlm_download->id, '_dlm_meta_extra', true) . '</p>'; } ?></a> - <?php echo $dlm_download->get_the_filesize(); ?>
	<br />
	<span class="dl-date-time"><i class="fa fa-clock-o"></i> <?php echo date_i18n( get_option( 'date_format' ), strtotime( $dlm_download->post->post_modified ) ); ?></span> <span class="dl-stats"><i class="fa fa-bar-chart"></i> Nedladdad <?php echo sprintf( _n( '1 gång', '%d gånger', $dlm_download->get_the_download_count(), 'dlm_page_addon' ), $dlm_download->get_the_download_count() ); ?></span>
</div>
